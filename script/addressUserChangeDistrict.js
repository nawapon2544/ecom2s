function changeDistrict(evt) {
  setStateProp(getAddressSectionEl(), 'data-district', 'true')
  setStateProp(getAddressSectionEl(), 'data-subdistrict', 'false')
  enableAddressTarget('subdistrict')
  const districtEl = $(evt.target)
  const district = districtEl.val().trim()
  $.ajax({
    ...
    success: function (response) {
      if (validateErr(response)) {
        ...
        if (obj.result) {
          ...
          addressTabActive(getDistrictEl(), district)
          getTargetControlEl().attr('data-state', 'address-subdistrict')
