$('#update-product').click(function () {
  ...
  productForm.forEach((fd) => {
    ...
    if (formtype == 'file') {
      const afterFiles = input[0][0].files.length
      const beforeFiles = input[1].attr('data-img').trim()
      const beforeFilesLength = beforeFiles != '' ? beforeFiles.split(',').length : 0

      if (afterFiles == 0 && beforeFilesLength == 0) {
        emptyCount++
        formValidate(true, validate, msg)
  ...
  if (emptyCount == 0) {
    ...
    formData.append('product_detail', $('#product-detail').val().trim())
    ...
    formData.append('product_before_img', productBeforeImg.attr('data-img'))
    formData.append('product_remove_img', productBeforeImg.attr('data-remove-img'))
    formData.append('product_delivery_cost', JSON.stringify(delivery_cost))
    formData.append('product_id', $(this).attr('data-productId'))
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (s >= 0) {
          ...
          if (!result) {
            const msg = obj.msg
            errMessage('อัพเดตข้อมูลสินค้า', msg, '')
          }

          if (result) {
            displaySuccess('อัพเดตข้อมูลเรียบร้อย', true)
          }
        }
